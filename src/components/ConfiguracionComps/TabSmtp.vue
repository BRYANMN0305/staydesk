...
const probarConexion = async () => {
  ...
  try {
    const res = await fetch(`${API_URL}/configuracion/smtp/test`, {
      ...
      body: JSON.stringify({ servidor: form.value.servidor, puerto: form.value.puerto, usuario: form.value.usuario, contrasena: form.value.contrasena })
    })
    estadoConexion.value = res.ok ? 'ok' : 'error'
  } catch { estadoConexion.value = 'error' }
