<template>
  <div class="config-card">
    ...
    <div v-else class="roles-table">
      ...
      <div v-for="rol in rolesFiltrados" :key="rol.id_rol" class="table-row">
        ...
        <div class="rol-nombre">
          <span class="rol-dot" :style="{ background: colorRol(rol.id_rol) }"></span>
    ...
    <div v-if="errorGlobal" class="error-banner mt-3">
...
// Se asigna un color por índice de posición del rol en el array,
// garantizando que no se repita mientras haya menos de 12 roles.
const PALETA = [
...
// Mapa persistente { id_rol → color } para que el color no cambie
// al reordenar o filtrar la tabla.
const rolColorMap = ref({})

const asignarColores = (listaRoles) => {
  // Solo asignamos color a los que aún no lo tienen
  let nextIndex = Object.keys(rolColorMap.value).length
  for (const rol of listaRoles) {
    if (!rolColorMap.value[rol.id_rol]) {
      rolColorMap.value[rol.id_rol] = PALETA[nextIndex % PALETA.length]
      nextIndex++
    }
  }
}

const colorRol = (id_rol) => rolColorMap.value[id_rol] ?? '#6320EE'
...
const rolAccion          = ref(null)
const rolSesionAfectado  = ref(false)
...
const cargar = async () => {
  ...
  try {
    ...
    asignarColores(roles.value)   // ← asignar colores únicos tras cargar
...
const abrirModalEditar = async (rol) => {
  editando.value = true
  form.value = { id_rol: rol.id_rol, nombre: rol.nombre, descripcion: rol.descripcion || '', permisos_ids: [] }
  errorModal.value = ''
  modalAbierto.value = true
  await cargarTodosPermisos()
...
const cerrarModal = () => { modalAbierto.value = false; errorModal.value = '' }
  ...
  try {
    if (editando.value) {
      const res = await fetch(`${API_URL}/roles/${form.value.id_rol}/editar`, {
        ...
        body: JSON.stringify({ nombre: form.value.nombre, descripcion: form.value.descripcion }),
      })
      if (!res.ok) {
      ...
      // Asignar color inmediatamente al nuevo rol antes de recargar
      asignarColores([nuevoRol])
    ...
    if (editando.value) {
      ...
      if (token) {
        try {
          ...
          if (payload.id_rol === form.value.id_rol) rolSesionAfectado.value = true
        } catch {}
      }
    }
  } catch {
...
const eliminar = async () => {
  ...
  try {
    ...
    // Liberar el color del rol eliminado para que pueda reutilizarse
    delete rolColorMap.value[rolAccion.value.id_rol]
...
const generarDescripcion = () => {
  ...
  const NOMBRES = {
    RESERVAS: 'reservas', HABITACIONES: 'habitaciones', PISOS: 'pisos',
    HUESPEDES: 'huéspedes', USUARIOS: 'usuarios', ROLES: 'roles',
    CONFIGURACION: 'configuración', HOUSEKEEPING: 'housekeeping',
    MANTENIMIENTO: 'mantenimiento', NOTIFICACIONES: 'notificaciones',
    AUDITORIA: 'auditoría', REPORTES: 'reportes', DASHBOARD: 'dashboard',
    FACTURAS: 'facturas', SERVICIOS: 'servicios', RESERVA_SERVICIOS: 'servicios de reserva',
  }
...
/* Estados vacíos */
.state-empty    { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 32px; color: #9ca3af; font-size: 13px; }
...
.btn-sel-todo { font-size: 11px; font-weight: 600; color: #6320EE; background: none; border: none; cursor: pointer; font-family: 'Outfit',sans-serif; text-decoration: underline; padding: 0; }
...
/* Footer modal */
.d-flex { display: flex; } .gap-2 { gap: .5rem; } .flex-1 { flex: 1; } .mt-4 { margin-top: 1.25rem; }
...
/* Spinners */
.spinner-sm { width:13px; height:13px; border:2px solid rgba(255,255,255,0.3); border-top-color:#fff; border-radius:50%; animation:spin .7s linear infinite; display:inline-block; }
.spinner-sm-purple { width:13px; height:13px; border:2px solid rgba(99,32,238,0.2); border-top-color:#6320EE; border-radius:50%; animation:spin .7s linear infinite; display:inline-block; }
.spinner-lg { width:28px; height:28px; border:3px solid rgba(99,32,238,0.15); border-top-color:#6320EE; border-radius:50%; animation:spin .7s linear infinite; display:inline-block; }
...
.sesion-icon { display: flex; align-items: center; flex-shrink: 0; }
...
/* Generar descripción */
.desc-label-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 5px; }
