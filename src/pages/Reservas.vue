...
const crearReserva = async () => {
  ...
  if (!nombre || !apellido) {
    errorModal.value = "Nombre y apellido del huésped son obligatorios.";
    return;
  }
  if (!huespedEncontrado.value && !documento) {
    errorModal.value =
      "El documento es obligatorio para registrar un huésped nuevo.";
    return;
  }
  if (!id_habitacion || !fecha_entrada || !fecha_salida) {
    errorModal.value = "Completa todos los datos de la reserva.";
    return;
  }
