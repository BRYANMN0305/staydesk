<template>
  <div class="dashboard-layout">
    ...
    <div class="d-flex flex-column overflow-hidden flex-grow-1 min-w-0">
      ...
      >
        <div class="d-flex align-items-center gap-3">
          <button
            class="d-md-none border-0 bg-transparent p-1"
          ...
          <div>
            <h1 class="fw-bold mb-0" style="font-size: 20px; color: #211a1d">
              {{ tituloHeader }}
            </h1>
            <p class="text-muted mb-0 text-capitalize" style="font-size: 12px">
              {{ today }}
            </p>
          </div>
        </div>
        <div class="d-flex align-items-center gap-2">
          <div
            class="search-wrap d-none d-md-flex align-items-center gap-2 rounded-3 px-3"
          >
            <svg
              width="15"
              height="15"
      ...
      >
        <template v-if="activeTab === 'dashboard'">
          <!-- ESTADÍSTICAS -->
          <div
            class="bg-white rounded-3 border p-3 p-md-4 mb-4"
            style="
              border-color: rgba(99, 32, 238, 0.07) !important;
          ...
          >
            <div class="mb-4">
              ...
              <h2 class="fw-bold mb-0" style="font-size: 22px; color: #211a1d">
            ...
            <div class="row g-2 g-sm-3 g-lg-3">
              <div class="col-12 col-sm-6 col-lg-3">
                <Estadistica_Card
                  ...
                  :value="String(estadisticas.habitaciones_disponibles)"
                  color="#16a34a"
              ...
              <div class="col-12 col-sm-6 col-lg-3">
                <Estadistica_Card
                  ...
                  :value="String(estadisticas.habitaciones_total)"
                  color="#6320EE"
                  bg="rgba(99,32,238,0.12)"
              ...
              <div class="col-12 col-sm-6 col-lg-3">
                <Estadistica_Card
                  label="Check-ins hoy"
                  :value="String(estadisticas.checkins_hoy)"
                  color="#d97706"
              ...
              <div class="col-12 col-sm-6 col-lg-3">
                <Estadistica_Card
                  ...
                  :value="String(estadisticas.checkouts_hoy)"
                  color="#dc2626"
          ...
          >
            ...
            >
              <div>
                <h2 class="fw-bold mb-0" style="font-size: 17px">
                  Habitaciones
                </h2>
          ...
          <!-- FACTURACIÓN -->
          <div
            class="bg-white rounded-3 border p-3 p-md-4"
            style="
              border-color: rgba(99, 32, 238, 0.07) !important;
              box-shadow: 0 2px 12px rgba(33, 26, 29, 0.05);
            "
          >
            ...
            >
              ...
              <button class="btn-outline" @click="setTab('facturas')">
                Ver todo
              </button>
            </div>
            <div
              v-if="cargandoFacturas"
              class="text-muted text-center py-3"
              style="font-size: 13.5px"
            >
              Cargando facturas...
            ...
            <template v-else>
              <div class="d-none d-md-block">
                <div
                  class="row g-0 px-3 py-2 text-uppercase fw-semibold"
                  style="
                    font-size: 11.5px;
                    color: #9ca3af;
                ...
                <div
                  class="row g-0 align-items-center px-3 py-3 table-row"
                  v-for="bill in billing.slice(0, 6)"
                  :key="bill.id"
                  style="border-bottom: 1px solid rgba(99, 32, 238, 0.05)"
                >
                  <div class="col-4 d-flex align-items-center gap-2">
                    ...
                    >
                      {{ bill.guest?.[0]?.toUpperCase() || "?" }}
                    </div>
                    <span class="fw-medium" style="font-size: 13.5px">{{
                      bill.guest
                    }}</span>
                  </div>
                  <div class="col-2 text-secondary" style="font-size: 13.5px">
                    Reserva #{{ bill.room }}
                  </div>
                  <div class="col-2 text-secondary" style="font-size: 13.5px">
                    {{ bill.checkout }}
                  </div>
                  <div class="col-2 fw-bold" style="font-size: 14px">
                    ${{ bill.total.toLocaleString() }}
                  </div>
                  <div class="col-2">
                    <span
                      class="rounded-pill px-2 py-1 fw-semibold"
                      style="font-size: 11.5px"
                      :class="bill.paid ? 'status-paid' : 'status-pending'"
                      >{{ bill.paid ? "Pagada" : "Pendiente" }}</span
                    >
                  </div>
                </div>
              </div>
        ...
        <Habitaciones
          v-if="activeTab === 'habitaciones'"
          @habitacion-actualizada="cargarHabitaciones"
        />
        <Facturas v-if="activeTab === 'facturas'" />
        <Reportes v-if="activeTab === 'reportes'" />
        <Reservas
          v-show="activeTab === 'reservas'"
          @reserva-actualizada="cargarHabitaciones"
          @checkin-iniciado="onCheckinIniciado"
        />
        <Servicios
          v-show="activeTab === 'servicios'"
          :reservas-checkin="reservasEnCheckin"
    ...
    >
      <div
        class="bg-white rounded-4 p-4 position-relative overflow-hidden modal-box"
      ...
      >
        ...
        <div class="row g-3 mb-4">
          <div class="col-6">
            <span
              class="d-block text-muted text-uppercase"
          ...
          <div class="col-6">
            <span
              ...
              >Precio</span
            ><strong style="font-size: 14px"
              >${{ selectedRoom.price.toLocaleString() }}/noche</strong
            >
          </div>
        ...
        <div class="d-flex gap-2">
          <button
            class="modal-btn-primary flex-grow-1 rounded-3 border-0 fw-semibold"
            @click="irAReservas(selectedRoom)"
    ...
    >
      <div class="bg-white rounded-4 p-4 position-relative logout-modal-box">
        ...
        <div class="d-flex align-items-center gap-3 mb-3 mt-1">
          <div
            class="d-flex align-items-center justify-content-center rounded-circle flex-shrink-0 logout-icon-wrap"
          >
            <svg
              width="20"
              height="20"
          ...
          <div>
            <h3 class="fw-bold mb-0" style="font-size: 17px; color: #211a1d">
              Cerrar sesión
            </h3>
        ...
        <div class="d-flex gap-2">
          <button
            class="logout-btn-cancel flex-grow-1 rounded-3 fw-semibold"
            @click="showLogoutModal = false"
          >
            Cancelar
          </button>
          <button
            class="logout-btn-confirm flex-grow-1 rounded-3 fw-semibold"
            @click="handleLogout"
          >
            Confirmar
          </button>
...
watch(
  () => route.params.vista,
  (vista) => {
    if (vista && vista !== activeTab.value) activeTab.value = vista;
  },
);
...
const cargarPisos = async () => {
  try {
    const res = await apiFetch(`${import.meta.env.VITE_API_URL}/pisos`);
  ...
  try {
    ...
    rooms.value = lista
      ...
      .map((h) => ({
        ...
        floor:
          h.piso?.numero ||
          pisos.value.find((p) => p.id_piso === h.id_piso)?.numero ||
          h.id_piso ||
          1,
    ...
    estadisticas.value.habitaciones_disponibles = rooms.value.filter(
  ...
  try {
    ...
    billing.value = data
      ...
      .map((f) => ({
        id: f.id_factura,
        guest:
          `${f.nombre_huesped || ""} ${f.apellido_huesped || ""}`.trim() ||
          "Sin nombre",
        room: f.id_reserva,
        checkout: new Date(f.fecha).toLocaleDateString("es-CO", {
          day: "numeric",
          month: "short",
        }),
...
function onCheckinIniciado(reserva) {
  const yaExiste = reservasEnCheckin.value.find(
...
const filteredRooms = computed(() => {
  ...
  return rooms.value.filter(
    (r) =>
      ...
      (r.guest &&
        r.guest.toLowerCase().includes(searchQuery.value.toLowerCase())),
  );
});
...
const handleTabChange = (tab) => {
  setTab(tab)
  if (tab === 'dashboard') {
    cargarPisos(); cargarHabitaciones(); cargarEstadisticas(); cargarFacturas()
  }
  if (tab === 'servicios') {
    sincronizarCheckins()
  }
...
const sincronizarCheckins = async () => {
  try {
    const res = await apiFetch(`${import.meta.env.VITE_API_URL}/reservas`)
    if (!res.ok) return

    const data = await res.json()

    reservasEnCheckin.value = data
      .filter((r) => r.estado?.toUpperCase() === "CHECKIN")
      .map((r) => ({
        ...r,
        estado: r.estado || "CHECKIN",
        _confirmado: r._confirmado || false,
    ...
    console.error("Error sincronizando checkins:", e)
  }
}
