<template>
    <div class="container-fluid min-vh-100 p-0">
        <div class="row g-0 min-vh-100">
            ...
            <!-- LADO IZQUIERDO -->
            <div
                class="col-12 col-md-5 col-xl-4 left-panel d-flex flex-column justify-content-center align-items-center px-4 px-md-5">
                ...
                <div class="card login-card w-100">
                    <div class="card-body p-4">
                        ...
                        <form @submit.prevent="login" novalidate>
                            ...
                            <!-- Error de API -->
                            <div v-if="apiError"
                                class="alert-error mb-3 rounded-3 px-3 py-2 d-flex align-items-center gap-2">
...
// VALIDACIONES
const validateEmail = () => {
    errors.email = !email.value ? 'El usuario es requerido' : ''
}
...
const login = async () => {
    ...
    try {
        ...
        localStorage.setItem('rol', rol === 'administrador' ? 'admin' : 'empleado')
...
.left-panel {
    background: linear-gradient(155deg, #211A1D 0%, #3a1060 45%, #6320EE 100%);
    min-height: 100vh;
}
...
.login-card {
    ...
    border: none;
    background: #F8F0FB;
    box-shadow: 0 28px 60px rgba(33, 26, 29, 0.45);
}
...
.btn-login {
    padding: 13.5px;
    background: linear-gradient(135deg, #6320EE, #8075FF 55%, #9a8fff);
    color: #fff;
    font-size: 15px;
    font-weight: 700;
    border: none;
    border-radius: 10px;
    cursor: pointer;
    box-shadow: 0 6px 20px rgba(99, 32, 238, 0.42);
    transition: transform .2s, box-shadow .2s;
}

.btn-login:hover:not(:disabled) {
    transform: translateY(-2px);
    box-shadow: 0 10px 28px rgba(99, 32, 238, 0.52);
}
...
.right-panel {
    background: url('https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=1400&q=85') no-repeat center / cover;
    position: relative;
    min-height: 100vh;
}
...
.right-overlay {
    position: absolute;
    inset: 0;
    background: linear-gradient(to top, rgba(33, 26, 29, 0.92) 0%, rgba(99, 32, 238, 0.25) 55%, transparent 100%);
}
