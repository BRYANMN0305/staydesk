  ...
  <div class="servicios-wrapper">
    ...
    <div class="header-area">
      <div>
        ...
        <h2 class="fw-bold mb-0" style="font-size: 25px; color: #211a1d">
          Servicios
        </h2>
      </div>
        ...
        <div class="card-derecha">
          <template v-if="editandoPrecio === s.id_servicio">
            ...
            <div class="precio-edit-btns">
              <button
                class="btn-precio-ok"
                @click="guardarPrecio(s)"
                :disabled="guardandoPrecio === s.id_servicio"
                title="Guardar"
              >
                ✓
              </button>
              <button
                class="btn-precio-cancel"
          ...
          <template v-else>
            <div class="precio-row">
              <span class="precio-valor"
                >${{ formatearPrecio(s.precio_unidad) }}</span
              >
              <button
    ...
    <div class="seccion-checkin">
      ...
      <div v-else class="grid-checkin">
        <div
          v-for="reserva in reservasEnCheckin"
        ...
        >
          <div class="cc-top">
            <div class="cc-hab-badge">
              ...
              <span class="cc-hab-num"
                >Hab. {{ reserva.habitacion?.numero }}</span
              >
            </div>
            <span v-if="reserva._checkout" class="cc-badge-checkout"
              >✓ Check-out</span
            >
            <span v-else-if="reserva._confirmado" class="cc-badge-ok"
              >✓ Confirmado</span
            >
          ...
          <div class="cc-huesped">
            <span class="cc-huesped-nombre"
              >{{ reserva.huesped?.nombre }}
              {{ reserva.huesped?.apellido }}</span
            >
            <span class="cc-huesped-fecha"
              >Entrada: {{ formatFecha(reserva.fecha_entrada) }}</span
            >
          </div>

          <div
            class="cc-servicios-lista"
            v-if="(reserva._serviciosAsignados || []).length > 0"
          >
            <div
              v-for="(item, idx) in reserva._serviciosAsignados"
              :key="idx"
              class="cc-servicio-item"
            >
              <span
                class="cc-srv-icono"
                v-html="renderIcono(item.nombre)"
              ></span>
              <span class="cc-srv-nombre">{{ item.nombre }}</span>
              <div class="cc-srv-controles">
                <button
                  class="cc-qty-btn"
                  @click="decrementarCantidad(reserva, idx)"
                  :disabled="reserva._checkout"
                >
                  −
                </button>
                ...
                <button
                  class="cc-qty-btn"
                  @click="incrementarCantidad(reserva, idx)"
                  :disabled="reserva._checkout"
                >
                  ＋
                </button>
              </div>
              <span class="cc-srv-subtotal"
                >${{
                  formatearPrecio(item.precio_unidad * item.cantidad)
                }}</span
              >
              <button
                class="cc-srv-remove"
                @click="quitarServicio(reserva, idx)"
                :disabled="reserva._checkout"
                title="Quitar"
              >
                ✕
          ...
          >
            ...
            <span class="cc-total-valor"
              >${{ formatearPrecio(calcularTotal(reserva)) }}</span
            >
          </div>

          <button
            v-if="!reserva._checkout"
            class="btn-add-srv"
            @click="abrirSelectorServicio(reserva)"
          >
          ...
          <div class="cc-acciones" v-if="!reserva._checkout">
            <button
              class="btn-sin-servicio"
              @click="confirmarSinServicios(reserva)"
              :disabled="reserva._guardando"
            >
              <span>✓</span> Sin servicios
            </button>
            <button
              class="btn-confirmar-srv"
              @click="confirmarServicios(reserva)"
              :disabled="
                reserva._guardando ||
                (reserva._serviciosAsignados || []).length === 0
              "
            >
          ...
          <div class="cc-checkout-msg" v-if="reserva._checkout">
            <span>✓</span> Check-out realizado — no se pueden modificar
            servicios
          </div>

          <div
            class="cc-confirmado-msg"
            v-if="reserva._confirmado && !reserva._checkout"
          >
      ...
      <div v-if="mostrarModal" class="modal-overlay" @click.self="cerrarModal">
        <div class="modal-box">
          ...
          <div class="campo">
            ...
            <div
              class="input-prefix"
              :class="{ 'input-prefix-error': errores.precio_unidad }"
            >
              <span>$</span>
              <input
                v-model.number="nuevoServicio.precio_unidad"
                type="number"
                placeholder="0"
                min="0"
            ...
            <span v-if="errores.precio_unidad" class="error-msg">{{
          ...
          <div class="campo" v-if="nuevoServicio.nombre">
            ...
            <div class="icono-preview">
              ...
              <span class="icono-preview-texto"
                >Generado automáticamente según el nombre</span
              >
            </div>
          </div>
    ...
    <ModalFormulario
      v-if="servicioAEliminar"
      titulo="¿Eliminar servicio?"
      :subtitulo="`Se eliminará ${servicioAEliminar.nombre}. Esta acción no se puede deshacer.`"
      :cargando="eliminando"
      textoConfirmar="Eliminar"
      :peligroso="true"
      @cerrar="servicioAEliminar = null"
      @confirmar="eliminarServicio"
    />
    ...
    <ModalFormulario
      ...
      :subtitulo="errorEliminar"
      textoConfirmar="Entendido"
      :peligroso="true"
      @cerrar="errorEliminar = ''"
      @confirmar="errorEliminar = ''"
    />
    <!-- Modal selector de servicio para habitación -->
    <Transition name="modal">
      <div
        v-if="modalSelectorVisible"
        class="modal-overlay"
        @click.self="modalSelectorVisible = false"
      >
        <div class="modal-box modal-selector">
          <h3 class="modal-titulo">Agregar servicio</h3>
          <p class="selector-sub">
            Hab. {{ reservaSeleccionada?.habitacion?.numero }} —
            {{ reservaSeleccionada?.huesped?.nombre }}
          </p>
          ...
          <div class="modal-acciones">
            <button class="btn-cancelar" @click="modalSelectorVisible = false">
              Cerrar
            </button>
          </div>
        </div>
...
function renderIcono(nombre) {
  ...
  return `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="${color}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="${path}"/></svg>`;
}
...
function persistirReservas(lista) {
  ...
  for (const r of lista) {
    if (r._checkout) continue;
    mapa[r.id_reserva] = {
      id_reserva: r.id_reserva,
      fecha_entrada: r.fecha_entrada,
      habitacion: r.habitacion,
      huesped: r.huesped,
      estado: r.estado || "CHECKIN",
...
// Un solo computed limpio
const reservasEnCheckin = computed(() => {
  const estadosActivos = new Set(["CHECKIN", "OCUPADO"]);
  return props.reservasCheckin
    .map((r) => ({
      ...r,
      estado: r.estado || "CHECKIN",
      ...getEstado(r.id_reserva),
    }))
    .filter((r) => {
      const estado = (r.estado || "CHECKIN").toUpperCase();
      const enCheckout = checkoutStore.idsCheckout.includes(
        Number(r.id_reserva),
      );
      return !r._checkout && !enCheckout && estadosActivos.has(estado);
    });
});
...
function agregarServicioAReserva(servicio) {
  ...
  const existente = estado._serviciosAsignados.find(
  ...
  mostrarToast(
    `${servicio.nombre} agregado a Hab. ${reserva.habitacion?.numero}`,
  );
}
...
function calcularTotal(reserva) {
  return (reserva._serviciosAsignados || []).reduce(
    (acc, s) => acc + s.precio_unidad * s.cantidad,
    0,
  );
}
...
async function confirmarServicios(reserva) {
  ...
  try {
    const res = await fetch(
      `${API_URL}/reserva/${reserva.id_reserva}/servicios`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json", ...authHeaders() },
        body: JSON.stringify({
          servicios: estado._serviciosAsignados.map((s) => ({
    ...
    if (!res.ok) throw new Error("Error al guardar los servicios");
    estado._confirmado = true;
    setTimeout(() => {
      estado._confirmado = false;
    }, 3000);
    mostrarToast(
      `Servicios guardados para Hab. ${reserva.habitacion?.numero} ✓`,
    );
...
async function guardarPrecio(servicio) {
  if (!precioEditando.value || precioEditando.value <= 0) {
  ...
  try {
    const res = await fetch(
      `${API_URL}/actualizar_precio/servicio/${servicio.id_servicio}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json", ...authHeaders() },
...
function validar() {
  ...
  if (!nuevoServicio.precio_unidad || nuevoServicio.precio_unidad <= 0) {
    errores.precio_unidad = "Ingresa un precio válido.";
    ok = false;
  }
  return ok;
}
...
async function eliminarServicio() {
  ...
  try {
    ...
    servicioAEliminar.value = null;
    errorEliminar.value =
      "Ocurrió un error de conexión al intentar eliminar el servicio.";
  } finally {
