...
const cargarReservas = async () => {
  cargando.value = true
  try {
    const res = await fetch(`${API}/listar_reservas`, { headers: authHeaders() })
    ...
    await Promise.all(
      reservasData
        ...
        .map(async r => {
          try {
            const resH = await fetch(`${API}/listar_huesped/id/${r.id_huesped}`, { headers: authHeaders() })
            if (resH.ok) {
...
const cargarHabitaciones = async () => {
  try {
    const res = await fetch(`${API}/listar_todas_habitaciones`, { headers: authHeaders() })
...
const buscarHuesped = async () => {
  ...
  try {
    const res = await fetch(`${API}/listar_huesped/documento/${busquedaDoc.value.trim()}`, { headers: authHeaders() })
    if (res.ok) {
...
const crearReserva = async () => {
  ...
  try {
    ...
    if (huespedEncontrado.value) {
      await fetch(`${API}/actualizar_huesped/${idHuesped}`, { method: 'PUT', headers: authHeaders(), body: JSON.stringify({ nombre, apellido, telefono, email }) })
    } else {
      const resH = await fetch(`${API}/crear_huesped`, { method: 'POST', headers: authHeaders(), body: JSON.stringify({ nombre, apellido, documento, telefono, email }) })
    ...
    const resR = await fetch(`${API}/crear_reserva`, { method: 'POST', headers: authHeaders(), body: JSON.stringify({ id_habitacion: Number(id_habitacion), id_huesped: idHuesped, fecha_entrada: fecha_entrada + 'T00:00:00', fecha_salida: fecha_salida + 'T00:00:00' }) })
    if (!resR.ok) {
...
const confirmar = async (id) => {
  procesando.value = id
  try {
    const res = await fetch(`${API}/confirmar_reserva/${id}`, { method: 'PATCH', headers: authHeaders() })
...
const cancelar = async () => {
  ...
  try {
    await fetch(`${API}/cancelar_reserva/${reservaACancelar.value.id_reserva}`, { method: 'PATCH', headers: authHeaders() })
...
const checkin = async (reserva) => {
  ...
  try {
    const resCheckin = await fetch(`${API}/checkin_reserva/${reserva.id_reserva}`, { method: 'PATCH', headers: authHeaders() })
    if (!resCheckin.ok) {
...
const checkout = async (id) => {
  procesando.value = id
  try {
    await fetch(`${API}/checkout_reserva/${id}`, { method: 'PATCH', headers: authHeaders() })
